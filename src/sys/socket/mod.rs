...
impl<'a> ControlMessage<'a> {
    ...
    fn len(&self) -> usize {
        match *self {
            ...
            #[cfg(any(target_os = "android", target_os = "linux"))]
            ControlMessage::AlgSetIv(iv) => mem::size_of_val(&iv) + iv.len(),
            #[cfg(any(target_os = "android", target_os = "linux"))]
...
pub fn sockaddr_storage_to_addr(addr: &sockaddr_storage, len: usize) -> Result<SockAddr> {
    ...
    match c_int::from(addr.ss_family) {
        ...
        libc::AF_UNIX => {
            ...
            let sun = unsafe { *(addr as *const _ as *const sockaddr_un) };
